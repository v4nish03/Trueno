...
@router.post("/{producto_id}/mover-bodega-tienda")
def mover_bodega_tienda(
    producto_id: int,
    cantidad: int = Query(..., gt=0, description="Cantidad a mover de bodega a tienda"),
    db: Session = Depends(get_db)
):
    """Mueve stock interno desde bodega a tienda para disponibilidad de venta."""
    try:
        return producto_service.mover_stock_bodega_a_tienda(db, producto_id, cantidad)
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
